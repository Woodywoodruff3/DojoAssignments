...
def submit():
	...
	if len(request.form['email']) > 1 and len(request.form['email']) > 1 and len(request.form['first_name']) > 1 and len(request.form['last_name']) > 1 and len(request.form['password']) > 1 and len(request.form['confirm_password']) > 1 and EMAIL_REGEX.match(request.form['email']) and len(request.form['password']) > 7 and len(request.form['confirm_password']) > 7 and request.form['password'] == request.form['confirm_password']:
